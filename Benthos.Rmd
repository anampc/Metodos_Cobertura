...
Coral_Cover_model<-glmer(Coral/100 ~ 1 + Zona * Método + 
                          (1|Zona:Sitio) + (1|Buzo), 
                        data = General.data, family="binomial")
...
```{r}
Richness_model<-lmer(Riqueza..S. ~ Zona * Método  +  (1|Zona:Sitio) + (1|Buzo), data = Riqueza.data)
...
Coral_Richness<- ggplot(Riqueza.data) + facet_grid(~Transecto)+
  MyTheme+ scale_shape_manual(values=c(21,22,25))+ 
  stat_summary(aes(x=Método, y=Riqueza..S., fill=Método, group=Método),
                 fun.data = "mean_cl_boot", geom = "bar", 
                 position=position_dodge(width=0.8), alpha=0.8)+
   stat_summary(aes(x=Método, y=Riqueza..S., group=Método, color=Método),
                 fun.data = "mean_cl_boot", geom = "errorbar", 
                 position=position_dodge(width=0.8))+
   geom_jitter( aes (x=Método, y=Riqueza..S., shape=Buzo, 
...
```{r}
Time_model<-lmer(Riqueza..S. ~ Zona * Método  +  (1|Zona:Sitio) + (1|Buzo), data = Riqueza.data)
...
Method_time<- ggplot(Time.data) +
  facet_grid(~Transecto, switch = "both")+
  MyTheme+ scale_shape_manual(values=c(21,22,25))+ 
  
   stat_summary(aes(x=Método, y=Tiempo.Total..min.,
                   fill=Método, group=Método),
                 fun.data = "mean_cl_boot", geom = "bar",
                 position=position_dodge(width=0.8), alpha=0.8)+
   stat_summary(aes(x=Método, y=Tiempo.Total..min.,
                    group=Método, color=Método),
                 fun.data = "mean_cl_boot", geom = "errorbar",
                 position=position_dodge(width=0.8))+
  # geom_jitter( aes (x=Método, y=minutos,
  #                   shape=Buzo,
  #                   fill=Método, group=Método), alpha=0.5, 
  #              data = Time.data[Time.data$Fase=="Tiempo.Campo..min.", ])+
  geom_jitter( aes (x=Método, y=Tiempo.Total..min.,
                    shape=Buzo,
                    fill=Método, group=Método), alpha=1, 
               data = Time.data[Time.data$Fase=="Tiempo.Campo..min.", ])+
  
  # geom_jitter( aes (x=Método, 
  #                   y=Tiempo.Campo..min.+ Tiempo.Laboratorio..min.,
  #                   shape=Buzo,
  #                     fill=Método, group=Método), alpha=0.1)+
  scale_y_continuous(limits = c(0,60),
                      expand = c(0.01, 0.01),
                      breaks = seq(0, 60, 15),
                      name=expression("Tiempo total (min)"))+
  theme(legend.position="none")
Method_time
```
...
Figure<-ggarrange(Coral_Cover, Algae_Cover, Coral_Richness, Method_time,
          labels = c("A", "B", "C", "D"),
          ncol = 1, nrow = 4, heights=c(1.2, 0.8, .8, .8, 1.2))
Figure
